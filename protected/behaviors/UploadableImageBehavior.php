...
class UploadableImageBehavior extends CActiveRecordBehavior
{
    ...
    public function getThumbsUrl()
    {
        if ( $this->thumbsUrl === null ) {
            $this->thumbsUrl = $this->saveUrl.'/'.lcfirst(get_class($this->owner)).'/thumbs';
        }
    ...
    public function getImageUrl($version = false)
    {
        if ($version) {
            ...
            return '/'.$this->saveUrl.'/'.lcfirst(get_class($this->owner)).'/'.$this->owner->getAttribute($this->attributeName);
        }
    }
}
